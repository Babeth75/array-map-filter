...
function getCirclesProperties(radiuses) {
  ...
  const tabCircle = radiuses.map(function(radius){

    let circumference = (2 * Math.PI * parseInt(radius)).toFixed(3);
    
    let surface = (Math.PI * parseInt(radius) * parseInt(radius)).toFixed(3);
